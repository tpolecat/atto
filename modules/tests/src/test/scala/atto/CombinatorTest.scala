...
object CombinatorTest extends Properties("Combinator") {
  ...
  // TODO: attempt
   ...
   property("discardLeft") = forAll { (n: Int, s: String) =>
    !(s.exists(_.isDigit)) ==> {
      discardLeft(int, string(s)).parseOnly(n.toString + s) match {
  ...
  property("discardRight") = forAll { (n: Int, s: String) =>
    !(s.exists(_.isDigit)) ==> {
      discardRight(string(s), int).parseOnly(s + n.toString) match {
        case ParseResult.Done("", `s`) => true
        case _ => false
      }
    }
  ...
  property("andThen") = forAll { (n: Int, s: String) =>
    !(s.exists(_.isDigit)) ==> {
      andThen(string(s), int).parseOnly(s + n.toString) match {
        case ParseResult.Done("", (`s`, `n`)) => true
        case _ => false
      }
    }
  ...
  property("orElse/left") = forAll { (n: Int, s: String) =>
    !(s.isEmpty || s.exists(_.isDigit)) ==> {
      ...
      orElse(p1, p2).parseOnly(n.toString + s) match {
  ...
  property("orElse/right") = forAll { (n: Int, s: String) =>
    !(s.exists(_.isDigit)) ==> {
      val p1: Parser[Either[Int, String]] = string(s).map(Right(_))
      val p2: Parser[Either[Int, String]] = int.map(Left(_))
      orElse(p1, p2).parseOnly(s + n.toString) match {
  ...
  property("either/left") = forAll { (n: Int, s: String) =>
    !(s.isEmpty || s.exists(_.isDigit)) ==> {
      either(int, string(s)).parseOnly(n.toString + s) match {
        case ParseResult.Done(`s`, Left(`n`)) => true
        case _ => false
      }
    }
  ...
  property("either/right") = forAll { (n: Int, s: String) =>
    !(s.isEmpty || s.exists(_.isDigit)) ==> {
      either(int, string(s)).parseOnly(s + n.toString) match {
        case ParseResult.Done(_, Right(`s`)) => true
        case _ => false
      }
    }
  ...
  property("cons") = forAll { (c: Char, s: String) =>
    ...
    p.parseOnly(c.toString + s).option === Some(NonEmptyList(c, s.toList))
  }
  ...
  property("manyUntil") = forAll { (s: String) =>
    (s.nonEmpty && s.indexOf("x") === -1) ==> {
      ...
      r sameAs ParseResult.Done("yz", List(s, s, s))
    }
  }
  ...
  property("choice/1") = forAll { (a: String, b: String, c: String) =>
    (a.nonEmpty && b.nonEmpty && c.nonEmpty) ==> {
      ...
      (p.parseOnly(a + b + c).done sameAs ParseResult.Done(b + c, a)) &&
      (p.parseOnly(b + c + a).done sameAs ParseResult.Done(c + a, b)) &&
      (p.parseOnly(c + a + b).done sameAs ParseResult.Done(a + b, c))
    }
  }
  ...
  property("count") = forAll { (c: Char, n0: Int) =>
    val n = (n0.abs % 10) + 1
    val s = c.toString * 20
    count(n, char(c)).parseOnly(s) sameAs ParseResult.Done(s drop n, List.fill(n)(c))
  }
